...
from .models import User, MessageBoard, Message, PrivateMessage, TrustedInstance, Alias, BannedPubkey, ContentExtensionRequest, ValidFileType, FileAttachment, Applet, AppletData, AppletCategory, HighScore, AppletSharedState
...
class UserAdmin(BaseUserAdmin):
    list_display = ('username', 'email', 'access_level', 'is_staff', 'is_banned', 'is_agent', 'pubkey_checksum')
    fieldsets = BaseUserAdmin.fieldsets + (
        ('BBS Info', {'fields': ('access_level', 'is_banned', 'is_agent', 'pubkey', 'nickname')}),
    )
    list_filter = ('is_staff', 'is_superuser', 'is_active', 'groups', 'is_banned', 'is_agent')
    readonly_fields = BaseUserAdmin.readonly_fields + ('pubkey_checksum',)
    actions = ['reload_agent_service']
    ...
    @admin.action(description='Reload selected agent service(s)')
    def reload_agent_service(self, request, queryset):
        reloaded_count = 0
        failed_count = 0
        for user in queryset:
            if user.is_agent:
                ...
                self.message_user(request, f"User '{user.username}' is not an agent.", level='WARNING')
        
        if reloaded_count > 0:
            self.message_user(request, f"Successfully reloaded {reloaded_count} agent service(s).", level='SUCCESS')
        if failed_count > 0:
            self.message_user(request, f"Failed to reload {failed_count} agent service(s). Check server logs.", level='ERROR')
