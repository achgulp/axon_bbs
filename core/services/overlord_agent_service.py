...
GAME_CONFIG = {
    ...
    "world_tick": {
        "interval_seconds": 30, # Reduced for faster testing
...
class OverlordAgentService:
    ...
    def process_game_events(self):
        ...
        for post in action_posts:
            ...
            if not player_data:
                post.agent_status = 'failed'; post.save()
                continue
            ...
            try:
                ...
                if drone and not drone.get('is_in_comms_range', True):
                    ...
                    post.agent_status = 'processed'; post.save()
                    continue
    ...
    def handle_stop_task(self, author, player_data, body):
        ...
        drone = next((d for d in player_data.get('drones', []) if d.get('id') == drone_id), None)
        if not drone: return
        self.handle_cancel_task(author, player_data, body) # Stop is just a cancel for now
        ...
        for player in all_players:
            ...
            for drone in player_data.get('drones', []):
                ...
                if not was_in_range and is_in_range:
                    # Drone just returned to comms range, dump its event buffer
    ...
    def process_drone_tick(self, drone, player_data):
        # ... (implementation as before) ...
        # Now, instead of posting events directly, buffer them if out of range
        # Example for cargo full:
        # if drone['inventory']['rawOre'] >= capacity:
        #   drone['status'] = 'Returning to Base'
        #   event = {"subject": "CARGO_FULL", "body": {"summary": f"Drone {drone['id']} cargo full, returning to base."}}
        #   if drone['is_in_comms_range']: self._post_agent_event(event['subject'], event['body'])
        #   else: drone.get('event_buffer', []).append(event)
        pass
