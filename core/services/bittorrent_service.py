    ...
    def get_private_key(self):
        ...
        if self._private_key is None:
            try:
                ...
                local_instance = None
                if local_onion:
                    local_instance = TrustedInstance.objects.filter(onion_url__contains=local_onion).first()
                ...
                if local_instance and local_instance.encrypted_private_key:
                    ...
                    if not local_instance:
                        local_instance = TrustedInstance.objects.create(onion_url=f"http://{local_onion}:8000" if local_onion else "", pubkey=public_key_pem, encrypted_private_key=encrypted_private)
                    else:
