...
class SyncService:
    ...
    def _download_done_callback(self, manifest, future):
        ...
        # Remove from currently_downloading set first
        content_hash = manifest.get('content_hash')
        self.currently_downloading.discard(content_hash)

        try:
            encrypted_data = future.result() # Check for exceptions and get the result
            if encrypted_data:
    ...
    def _schedule_download(self, manifest):
        ...
        # Use a partial to pass the manifest to the callback
    ...
    def _download_content(self, manifest: dict) -> bytes | None:
        # UPDATED: This method is now simpler. It only downloads and returns encrypted data.
        # The recursive call has been removed.
