...
# --- Auth & Identity Views (Largely Unchanged) ---
...
class PostMessageView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Prepare the raw message content for BitSync
            message_content = {
                "subject": subject,
                "body": body,
                "board": board.name,
            ...
            # Use the BitSyncService to create the manifest and store chunks
            if service_manager.bitsync_service:
                manifest = service_manager.bitsync_service.create_manifest_and_store_chunks(raw_data)
                
                # Save the message to the local database with its manifest
                ...
                logger.info(f"New message '{subject}' posted and prepared for BitSync.")
...
# --- NEW & UPDATED BitSync Protocol Views ---

@method_decorator(csrf_exempt, name='dispatch')
class SyncView(views.APIView):
    """
    Handles polling requests from trusted peers.
    Returns a list of manifests for content created since the 'since' timestamp.
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Find all messages with a manifest created after the peer's last sync
...
class BitSyncHasContentView(views.APIView):
    """
    Allows peers to check if this node has a specific piece of content.
    """
    permission_classes = [TrustedPeerPermission]

    def get(self, request, content_hash, *args, **kwargs):
        # Check if a message with the given content_hash in its manifest exists
        has_content = Message.objects.filter(manifest__content_hash=content_hash).exists()
        if has_content:
...
class BitSyncChunkView(views.APIView):
    """
    Serves a specific chunk of content to a trusted peer.
    """
    permission_classes = [TrustedPeerPermission]
        ...
        if chunk_path:
            try:
                with open(chunk_path, 'rb') as f:
                    chunk_data = f.read()
                return HttpResponse(chunk_data, content_type='application/octet-stream')
            except IOError:
