...
from core.models import MessageBoard, Message, Ig# Full path: axon_bbs/api/views.py
from rest_framework import generics, permissions, status, views
from rest_framework.response import Response
from rest_framework.parsers import MultiPartParser, FormParser, JSONParser
from django.http import HttpResponse, Http404, JsonResponse
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            pm_content = {
                ...
                "recipient_pubkey": recipient_pubkey, # UPDATED: Explicitly include recipient pubkey
                "subject": subject,
                "body": e2e_body
            }
            ...
            try:
                ...
                if decrypted_content_bytes:
                    ...
                    if isinstance(e2e_body, dict) and 'recipient_copy' in e2e_body:
                        ...
                        pm.decrypted_body = str(e2e_body) # Handle legacy or plain text bodies
                else:
...
class GetPublicKeyView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            return Response({"error": "Configuration error: Multiple local instances found."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)noredPubkey, BannedPubkey, TrustedInstance, Alias, ContentExtensionRequest, FileAttachment, PrivateMessage, FederatedAction
from core.services.identity_service import IdentityService, DecryptionError
from core.services.encryption_utils import derive_key_from_password, generate_checksum, generate_short_id, encrypt_with_public_key, decrypt_with_private_key
from core.services.service_manager import service_manager
from core.services.content_validator import is_file_type_valid
