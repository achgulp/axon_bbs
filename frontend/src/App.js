...
const PrivateMessageClient = ({ setNeedsUnlock, initialRecipient = null }) => {
  ...
  // UPDATED: State is now derived from the initialRecipient prop to prevent timing issues
  ...
  const fetchMessages = useCallback(() => {
    ...
    apiClient.get('/api/pm/list/')
      ...
      .catch(err => {
        if (err.response && err.response.data.error === 'identity_l// Full path: axon_bbs/frontend/src/App.js
import React, { useState, useEffect, useCallback } from 'react';
import apiClient from './apiClient';
import LoginScreen from './components/LoginScreen';
import RegisterScreen from './components/RegisterScreen';
