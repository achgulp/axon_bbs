...
// UPDATED: This component is heavily modified to support Inbox, Outbox, and Forwarding.
const PrivateMessageClient = ({ setNeedsUnlock, initialRecipient = null }) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      setView('outbox'); // Switch to outbox to see the sent message
    } catch (err) {
      if (err.response && err.response.data.error === 'identity_locked') {
        setNeedsUnlock(true);
      } else {
  ...
  const handleForward = () => {
    ...
    setShowCompose(true);
    setRecipientIdentifier(''); // Clear recipient for forwarding
    ...
    return (
      <div>
        <button onClick={() => setSelectedMessage(null)} className="mb-4 bg-gray-700 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded">
          ← Back to {view === 'inbox' ? 'Inbox' : 'Outbox'}
        </button>
        {view === 'outbox' && (
        ...
        <div className="bg-gray-800 p-4 rounded border border-gray-700">
          ...
          <p className="text-sm text-gray-400 mb-2">
            {view === 'inbox' ? `From: ${selectedMessage.author_username}` : `To: ${selectedMessage.recipient_display}`}
            {' on '}
  ...
  const columns = view === 'inbox'
    ? { id: 'From', value: msg => msg.author_username }
    : { id: 'To', value: msg => msg.recipient_display };
          ...
          <tbody>
            {messages.map(msg => (
              <tr key={msg.id} className="border-b border-gray-700 last:border-b-0 hover:bg-gray-700 cursor-pointer" onClick={() => setSelectedMessage(msg)}>
                ...
                <td className="p-3 text-gray-400">{columns.value(msg)}</td>
