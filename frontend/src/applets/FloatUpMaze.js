...
(async function() {
    const styles = `
        body { font-family: 'Arial', sans-serif; display: flex; justify-content: center; align-items: center; background-color: #1a202c; color: #e2e8f0; padding-top: 20px; }
    ...
    let upwardSpeed = 1.0, horizontalSpeed = 1.5;
    let assetsLoaded = false, images = {}, buttonImages = {}, bombImage = null;
    let animationFrameId = null;
    let approachingTopOutcome = null;

    let saucer = { active: false, x: 0, y: -30, width: 40, height: 20, targetY: canvas.height / 2 - 30, speed: 2, state: 'idle', beamActive: false, pullSpeed: 1.5 };
    
    const T = null; const K = '#000000'; const W = '#FFFFFF'; const G = '#808080'; const LG = '#D3D3D3'; const Y = '#FFD700'; const BR = '#A0522D'; const R = '#FF0000'; const DG = '#333333'; const OR = '#FFA500'; const FY = '#FFFF00'; const SC = '#C0C0C0'; const BC = 'rgba(173, 216, 230, 0.5)'; const GC = '#228B22'; const LGC = '#90EE90'; const TRUNK_C = '#8B4513';
    ...
    function loadAssets() { messageArea.textContent = "Loading Assets..."; let loadedCount = 0; const totalAssets = Object.keys(characters).length + 1; buttonImages.giraffe = document.getElementById('imgGiraffe'); buttonImages.elephant = document.getElementById('imgElephant'); buttonImages.zebra = document.getElementById('imgZebra'); function assetLoaded() { loadedCount++; if (loadedCount === totalAssets) { assetsLoaded = true; messageArea.textContent = "Select character & Start!"; startButton.disabled = false; resetGame(); } } for (const charName in characters) { const dataUrl = characters[charName].dataUrl; if (dataUrl) { images[charName] = new Image(); images[charName].onload = assetLoaded; images[charName].onerror = assetLoaded; images[charName].src = dataUrl; if (buttonImages[charName]) { buttonImages[charName].src = dataUrl; } } else { assetLoaded(); } } if (bombDataUrl) { bombImage = new Image(); bombImage.onload = assetLoaded; bombImage.onerror = assetLoaded; bombImage.src = bombDataUrl; } else { assetLoaded(); } }

    let sounds = {}; let audioInitialized = false;
    function initializeAudio() { if (audioInitialized) return Promise.resolve(); return new Promise((resolve, reject) => { if (typeof Tone === 'undefined') { const script = document.createElement('script'); script.src = "https://cdnjs.cloudflare.com/ajax/libs/tone/14.8.49/Tone.js"; script.onload = () => { setupTones(); resolve(); }; script.onerror = reject; document.head.appendChild(script); } else { setupTones(); resolve(); } }); }
    function setupTones() { sounds.collision = new Tone.Synth({ oscillator: { type: "square" }, envelope: { attack: 0.01, decay: 0.1, sustain: 0, release: 0.1 } }).toDestination(); sounds.win = new Tone.Synth({ oscillator: { type: "triangle" }, envelope: { attack: 0.01, decay: 0.2, sustain: 0.1, release: 0.2 } }).toDestination(); sounds.lose = new Tone.Synth({ oscillator: { type: "sawtooth" }, envelope: { attack: 0.01, decay: 0.3, sustain: 0, release: 0.2 } }).toDestination(); sounds.explosion = new Tone.NoiseSynth({ noise: { type: 'white' }, envelope: { attack: 0.005, decay: 0.1, sustain: 0, release: 0.1 } }).toDestination(); sounds.beam = new Tone.AMOscillator({frequency: 100, type: "sine", modulationType: "sine", harmonicity: 1.01}).toDestination(); sounds.beam.volume.value = -15; audioInitialized = true; }
    function playSound(soundName, action = 'play') { if (!audioInitialized || !sounds[soundName]) return; try { if (Tone.context.state !== 'running') { Tone.context.resume(); } if (soundName === 'beam') { if (action === 'start' && sounds.beam.state !== 'started') { sounds.beam.start(); } else if (action === 'stop' && sounds.beam.state === 'started') { sounds.beam.stop(); } } else { if (soundName === 'collision') sounds.collision.triggerAttackRelease("C3", "8n"); if (soundName === 'win') sounds.win.triggerAttackRelease("C5", "4n"); if (soundName === 'lose') sounds.lose.triggerAttackRelease("A2", "4n"); if (soundName === 'explosion') sounds.explosion.triggerAttackRelease(); } } catch (e) { /* silent fail */ } }
    
    function defineTrophy() { trophy = { x: Math.random() * (canvas.width - 20), y: Math.random() * 40 + 80, width: 20, height: 30 }; }
    function generateSafePath() { safePathSegments = []; let currentX = canvas.width / 2 - 20; const segHeight = canvas.height / 12; for (let i = 0; i < 12; i++) { const segY = canvas.height - (i + 1) * segHeight; let nextX = currentX + (Math.random() - 0.5) * 80; nextX = Math.max(0, Math.min(canvas.width - 40, nextX)); safePathSegments.push({ x: nextX, y: segY, width: 40, height: segHeight }); currentX = nextX; } }
    function isPositionValid(x, y, w, h) { const rect = { x, y, width: w, height: h }; if (x < 2 || x + w > canvas.width - 2 || y < 40 || y + h > canvas.height - 40) return false; if (trophy && checkCollision(rect, trophy)) return false; if (checkCollision(rect, { x: 0, y: canvas.height - 42, width: canvas.width, height: 42 })) return false; for (const mine of mines) { if (checkCollision(rect, mine)) return false; } for (const seg of safePathSegments) { if (checkCollision(rect, seg)) return false; } return true; }
    function placeMines() { mines = []; for (let i = 0; i < 5000 && mines.length < 15; i++) { const x = Math.random()*(canvas.width-24); const y = Math.random()*(canvas.height-24); if(isPositionValid(x, y, 24, 24)) mines.push({x,y,width:24,height:24,image:bombImage});}}
    function initializePlayer() { if (!selectedCharacter || !assetsLoaded) return; player = { x: canvas.width / 2 - 8, y: canvas.height - 21, width: 16, height: 16, image: images[selectedCharacter], dy: -1.0 }; }
    function clearCanvas() { ctx.clearRect(0, 0, canvas.width, canvas.height); }
    function drawTrophy() { if(trophy) { ctx.fillStyle = TRUNK_C; ctx.fillRect(trophy.x + 7, trophy.y + 18, 6, 12); ctx.fillStyle = GC; ctx.beginPath(); ctx.arc(trophy.x + 10, trophy.y + 10, 10, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = LGC; ctx.beginPath(); ctx.arc(trophy.x + 8, trophy.y + 8, 6, 0, Math.PI * 2); ctx.fill(); } }
    function drawMines() { mines.forEach(m => { if (m.image?.complete) ctx.drawImage(m.image, m.x, m.y, m.width, m.height); }); }
    function drawPlayer() { if (player?.image?.complete && !isExploding && saucer.state !== 'beaming') ctx.drawImage(player.image, player.x, player.y, player.width, player.height); }
    ...
    function updatePlayerPosition() { if (!player || isGameOver || isExploding) return; if (trophy && checkCollision(player, trophy) && saucer.state !== 'beaming') return handleWin(); if (saucer.state === 'beaming') { const dX = saucer.x + 20 - (player.x + 8), dY = saucer.y - (player.y + 8); const dist = Math.sqrt(dX*dX + dY*dY); if (dist < 16) { playSound('beam', 'stop'); player = null; saucer.active = false; saucer.beamActive = false; saucer.state = 'idle'; return handleLose("Abducted!"); } player.x += (dX / dist) * 1.5; player.y += (dY / dist) * 1.5; return; } let nX = player.x; if (moveLeft) nX -= 1.5; if (moveRight) nX += 1.5; if (nX < 0 || nX + 16 > canvas.width) playSound('collision'); player.x = Math.max(0, Math.min(canvas.width - 16, nX)); const nY = player.y + player.dy; const triggerY = canvas.height/2; if(nY < triggerY && !saucer.active && approachingTopOutcome === null) { approachingTopOutcome = Math.random() < 0.3 ? 'saucer' : 'space'; if(approachingTopOutcome === 'saucer'){ saucer.active = true; saucer.state = 'entering'; saucer.y = -20; saucer.x = Math.max(0, Math.min(canvas.width-40, player.x-12)); saucer.targetY=triggerY-30;}} player.y=nY; if (approachingTopOutcome === 'space' && player.y <= 0) return handleLose("Floated into space!"); if (saucer.state !== 'beaming') { for(const m of mines) { if (checkCollision(player, m)) return triggerExplosion(player.x+8, player.y+8);}}}
    ...
    async function startGame() { if (!assetsLoaded || !selectedCharacter) return messageArea.textContent = !selectedCharacter ? "Select a character!":"Assets loading..."; if (!audioInitialized) await initializeAudio(); if (typeof Tone !== 'undefined') await Tone.start(); proceedWithStart(); }
    function proceedWithStart() { isGameRunning=true; isGameOver=false; isExploding=false; saucer.active=false; saucer.beamActive=false; saucer.state='idle'; approachingTopOutcome=null; messageArea.textContent = "Go! Reach the Tree!"; defineTrophy(); generateSafePath(); placeMines(); initializePlayer(); startButton.disabled=true; charButtons.forEach(b=>b.disabled=true); leftButton.disabled=false; rightButton.disabled=false; if(animationFrameId) cancelAnimationFrame(animationFrameId); gameLoop(); }
    ...
    function resetGame() { isGameOver=false; isGameRunning=false; isExploding=false; saucer.active=false; saucer.beamActive=false; saucer.state='idle'; playSound('beam','stop'); player=null; mines=[]; safePathSegments=[]; updateStatsDisplay(); messageArea.textContent=assetsLoaded?"Select character & Start!":"Loading..."; startButton.disabled=!assetsLoaded; charButtons.forEach(b=>b.disabled=!assetsLoaded); leftButton.disabled=true; rightButton.disabled=true; clearCanvas(); defineTrophy(); drawTrophy(); if(animationFrameId) { cancelAnimationFrame(animationFrameId); animationFrameId=null; } }
    
    startButton.addEventListener('click', startGame);
    charButtons.forEach(b => { b.addEventListener('click', () => { if(isGameRunning||isGameOver||!assetsLoaded) return; charButtons.forEach(btn=>btn.classList.remove('selected')); b.classList.add('selected'); selectedCharacter = b.dataset.char; messageArea.textContent=`Selected ${selectedCharacter}. Press Start!`; });});
    ...
    leftButton.addEventListener('mousedown', ()=>handleMove('Left',true)); leftButton.addEventListener('mouseup', ()=>handleMove('Left',false)); leftButton.addEventListener('mouseleave', ()=>handleMove('Left',false)); leftButton.addEventListener('touchstart', (e)=>{e.preventDefault();handleMove('Left',true);}); leftButton.addEventListener('touchend',()=>handleMove('Left',false));
