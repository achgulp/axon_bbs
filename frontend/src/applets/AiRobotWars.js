...
(async function () {
    try {
        const APPLET_VERSION = 'v2.9.46'; // Editable autopilot prompt (no recompile needed - edit in Commander Console)
        ...
        let autoPilotEnabled = false;     // Auto-pilot checkbox state
        let autoPilotActive = false;      // Auto-pilot currently running
        ...
        let commandQueryStartTime = 0;    // For measuring LLM latency

        // Default autopilot prompt (editable via UI)
        const DEFAULT_AUTOPILOT_PROMPT = `Commands: S=shoot C=charge D=dodge R=retreat M=mine
Tactics:
H30 E40 @90/200 L. R
H90 E850 @45/100 C. C
H85 E800 @-90/400 F. S
H70 E600 @120/250 G. M
H45 E40 @0/100 C. R
H{hp} E{energy} @{enemyAngle}/{enemyDist} {sit}.`;

        // Current autopilot prompt (loaded from localStorage or default)
        let autopilotPrompt = localStorage.getItem('autopilot_prompt') || DEFAULT_AUTOPILOT_PROMPT;
        ...
        `;
            ...
            // SSE/LLM Status Indicator (top-right)
            const sseIndicator = document.createElement('div');
            sseIndicator.id = 'sse-indicator';
            sseIndicator.style.cssText = 'position:absolute;top:10px;right:10px;background:rgba(55,65,81,0.95);padding:8px 16px;border-radius:8px;font-size:12px;z-index:100;border:1px solid #6b7280;';
            sseIndicator.innerHTML = `
                <div style="display:flex;align-items:center;gap:8px;">
                    <span id="sse-dot" style="width:8px;height:8px;background:#f59e0b;border-radius:50%;"></span>
                    <span id="sse-text" style="color:#e2e8f0;">Connecting...</span>
                </div>
                <div id="llm-stats" style="margin-top:4px;font-size:10px;color:#9ca3af;">LLM commands: 0</div>
            `;
            ui.appendChild(sseIndicator);
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    <span style="color:#60a5fa;font-weight:bold;">🤖 Commander Console</span>
                ...
                <div style="margin-top:8px;color:#6b7280;font-size:10px;">
                    💡 Tip: # prefix = direct command (e.g., #drive(0,100))
                </div>
                ...
                <!-- Autopilot Prompt Editor -->
                <div style="margin-top:12px;border-top:1px solid #374151;padding-top:12px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                        <span style="color:#60a5fa;font-weight:bold;font-size:11px;">⚙️ Autopilot Prompt</span>
                        <button id="toggle-prompt-editor" style="background:#374151;border:none;padding:4px 8px;border-radius:4px;color:#94a3b8;font-size:10px;cursor:pointer;">
                            Show ▼
                        </button>
                    </div>
                    <div id="prompt-editor-section" style="display:none;">
                        <textarea id="autopilot-prompt"
                            style="width:100%;height:120px;padding:8px;border:1px solid #374151;border-radius:5px;background:#0f172a;color:#e2e8f0;font-family:monospace;font-size:10px;resize:vertical;box-sizing:border-box;margin-bottom:6px;"
                            placeholder="Enter autopilot prompt template..."></textarea>
                        <div style="display:flex;gap:6px;">
                            <button id="save-prompt-btn" style="flex:1;padding:6px;background:#10b981;border:none;border-radius:4px;color:white;font-size:10px;cursor:pointer;font-weight:bold;">
                                💾 Save Prompt
                            </button>
                            <button id="reset-prompt-btn" style="flex:1;padding:6px;background:#6b7280;border:none;border-radius:4px;color:white;font-size:10px;cursor:pointer;">
                                🔄 Reset Default
                            </button>
                        </div>
                        <div style="margin-top:6px;color:#6b7280;font-size:9px;line-height:1.3;">
                            💡 Use {hp}, {energy}, {enemyAngle}, {enemyDist}, {sit} placeholders
                        </div>
                    </div>
                </div>
        `;
        ...
        // ═══════════════════════════════════════════════════════
        // SSE Connection for Real-time LLM Responses (v5.0)
        // ═══════════════════════════════════════════════════════

        let sseEventSource = null;
        let sseConnected = false;
        let llmCommandCount = 0; // Track LLM commands received

        function connectSSEStream() {
            if (sseEventSource) return; // Already connected

            // Get token from localStorage (same as chat and rest of BBS)
            const token = localStorage.getItem('token');
            if (!token) {
                debugLog('⚠️ No JWT token for SSE - using polling mode');

                // Update UI to show polling mode
                const sseDot = document.getElementById('sse-dot');
                const sseText = document.getElementById('sse-text');
                if (sseDot) sseDot.style.background = '#f59e0b'; // Amber/orange
                if (sseText) sseText.textContent = 'Polling Mode (no login)';

                // Retry SSE connection after a delay (only if autopilot still active)
                setTimeout(() => {
                    if (autoPilotActive && currentState === GameState.PLAYING) {
                        connectSSEStream();
                    }
                }, 5000);
                return;
            ...
            sseEventSource = new EventSource(`/api/chat/events?token=${token}`);

            sseEventSource.onopen = () => {
                sseConnected = true;
                debugLog('✅ SSE connected (60fps real-time)');

                // Update UI indicator
                const sseDot = document.getElementById('sse-dot');
                const sseText = document.getElementById('sse-text');
                if (sseDot) sseDot.style.background = '#10b981'; // Green
                if (sseText) sseText.textContent = 'SSE: Connected ✓';
            };
            ...
            sseEventSource.onmessage = (event) => {
                try {
                    ...
                    // Filter for RouterResponse
                    messages
                        .filter(m => m.subject === 'RouterResponse')
                        .forEach(msg => {
                            try {
                                const response = JSON.parse(msg.body);
                                handleSSERouterResponse(response);
                            } catch (e) {
                                debugLog(`Parse error: ${e.message}`);
                            }
                        });
                } catch (e) {
                    // Ignore (SSE sends pings)
                }
            };

            sseEventSource.onerror = (error) => {
                sseConnected = false;
                debugLog('❌ SSE error, retrying in 5s...');
                ...
                const sseDot = document.getElementById('sse-dot');
                const sseText = document.getElementById('sse-text');
                if (sseDot) sseDot.style.background = '#ef4444'; // Red
                if (sseText) sseText.textContent = 'SSE: Reconnecting...';

                // Only retry if autopilot still active and game playing
                setTimeout(() => {
                    if (autoPilotActive && currentState === GameState.PLAYING) {
                        connectSSEStream();
                    }
        ...
        function handleSSERouterResponse(response) {
            const queryId = response.query_id;

            if (queryId && queryId === evolveQueryId) {
                const elapsed = response.metadata?.processing_time || 0;
                debugLog(`⚡ SSE response: ${(elapsed * 1000).toFixed(1)}ms`);

                if (response.status === 'success') {
                    const answer = response.answer || '';
                    const validCommand = validateRobotCommand(answer);

                    if (validCommand) {
                        llmCommandCount++;
                        debugLog(`🤖 LLM COMMAND #${llmCommandCount}: "${answer}" → Executing: ${validCommand}`);

                        // Update UI counter
                        const llmStats = document.getElementById('llm-stats');
                        if (llmStats) llmStats.textContent = `LLM commands: ${llmCommandCount}`;

                        const robot = robots[0];
                        if (robot) {
                            executeAICommand(robot, validCommand);
                        }
                    } else {
                        debugLog(`❌ LLM RESPONSE INVALID: "${answer.substring(0, 50)}" (not a robot command)`);
                    }
                } else {
                    debugLog(`❌ LLM Error: ${response.error || 'Unknown'}`);
                }
        ...
        function showPreEvolutionDialog(blueWon, finalScore) {
            return new Promise((resolve) => {
                ...
                const resultText = blueWon ? '🏆 BLUE WON!' : '💀 BLUE LOST';
        ...
        async function requestPostMatchEvolution(blueWon, finalScore) {
            ...
            debugLog('═══ POST-MATCH EVOLUTION STARTING ═══');
            ...
            const historyStats = battleHistory.length > 0 ? {
                ...
                positions: battleHistory.slice(-5).map(h => h.blue ? `(${h.blue.x}, ${h.blue.y})` : '?').join(' → ')
            } : { totalSnapshots: 0 };
...
Output ONLY the JavaScript function. No explanations.`;
            ...
            indicator.innerHTML = '🧬 Evolving Script...<br><span style="font-size:0.7em;opacity:0.9;">Connecting to AI...</span>';
            ...
            try {
                ...
                if (indicator) {
                    indicator.innerHTML = '🧬 Evolving Script...<br><span style="font-size:0.7em;opacity:0.9;">Request sent! Waiting for strategy...</span>';
                }
                ...
                const pollInterval = setInterval(async () => {
                    ...
                    if (ind) {
                        ...
                        ind.innerHTML = `🧬 Evolving Script${dots}<br><span style="font-size:0.7em;opacity:0.9;">Generating strategy... ${remaining}s</span>`;
                    }
                                ...
                                try {
                                    ...
                                    if (response.query_id === queryId) {
                                        ...
                                        if (ind) {
                                            ...
                                            ind.innerHTML = '✅ Done!<br><span style="font-size:0.7em;">Script Created.</span>';
        ...
        async function handlePostMatchEvolutionResponse(response) {
            ...
            if (response.status === 'success' && response.answer) {
                ...
                if (result.success) {
                    ...
                    debugLog('✅ Post-match evolution complete! New script saved.');
                    appendAIOutput('✅ Script evolved! Ready for next battle.', true);
                    ...
                    // Show error dialog
                    alert('⚠️ AI-generated script had errors:\n\n' + result.error + '\n\nScript not applied.');
                }
            } else {
        ...
        function executeAICommand(robot, commandStr) {
            ...
            switch (cmd) {
                ...
                case 'EVADE':
                case 'DODGE': {
                    // D - Dodge: Random evasive maneuver at max speed
                    const randomAngle = (robot.heading + 90 + Math.random() * 180) % 360;
                    robot.drive(randomAngle, 100);
                    debugLog('[DODGE] Random evasive to ' + Math.round(randomAngle) + '°');
                    return true;
                }
                ...
                case 'ATTACK':
                case 'SHOOT': {
                    // S - Shoot: Fire at enemy position
                    const enemy = robots[1];
                    if (enemy) {
                        const dx = enemy.x - robot.x;
                        const dy = enemy.y - robot.y;
                        ...
                        debugLog('[SHOOT] Fire at ' + Math.round(angle) + '° / ' + Math.round(distance) + 'm');
                    }
                    return true;
                }

                case 'CHASE': {
                    // C - Chase: Fire at enemy AND drive toward them
                    const target = robots[1];
                    if (target) {
                        ...
                        // Fire first
                        robot.cannon(angle, distance);
                        // Then drive toward enemy at attack speed
                        robot.drive(angle, 90);
                        debugLog('[CHASE] Fire + drive to ' + Math.round(angle) + '°');
                    }
                    return true;
                }

                case 'RETREAT': {
                    // R - Retreat: Go to home base corner
                    const homeX = 100, homeY = 100; // Blue's home
                    const dx = homeX - robot.x;
                    const dy = homeY - robot.y;
                    const angle = Math.atan2(dy, dx) * (180 / Math.PI);
                    robot.drive(angle, 100); // Full speed retreat
                    debugLog('[RETREAT] Fleeing to base at ' + Math.round(angle) + '°');
                    return true;
                }

                case 'MINE': {
                    // M - Mine: Drop mine at current position
                    robot.dropMine();
                    debugLog('[MINE] Dropped at current position');
                    return true;
                }
                ...
                if (match) {
                    ...
                    switch (funcName) {
                        ...
                        case 'scan':
                            ...
                            appendAIOutput(`# scan(${args[0]}, ${args[1]}) → ${result}`, true);
                            return true;
                        case 'lidar':
                            const lidarResult = robot.lidar(args[0] || 0);
                            appendAIOutput(`# lidar(${args[0]}) → ${lidarResult}`, true);
                            return true;
                        case 'sonar':
                            const sonarResult = robot.sonar();
                            appendAIOutput(`# sonar() → angle:${sonarResult.angle}, dist:${sonarResult.distance}`, true);
                            return true;
                        case 'dropmine':
                        case 'mine':
                            robot.dropMine();
        ...
        async function requestEvolution() {
            ...
            if (evolveBtn) {
                ...
                evolveBtn.textContent = '⏳ Sending...';
            }

            debugLog('Requesting command: ' + evolveQueryId.slice(0, 8));
            appendAIOutput('Commander: ' + (commanderIntent || 'No orders'));

            // Create the prompt for the LLM - structured with examples
            const enemyAngle = Math.round(sitrep.enemy.angle);
            const enemyDist = Math.round(sitrep.enemy.distance);
            const hp = sitrep.hp;
            const energy = sitrep.energy;
            const orders = sitrep.commanderIntent || 'Fight!';

            // Determine tactical suggestion based on situation
            let situation = '';
            if (hp < 30) situation = 'CRITICAL HP - consider GOTO BASE';
            else if (energy < 200) situation = 'LOW ENERGY - conserve shots';
            else if (enemyDist < 150) situation = 'CLOSE RANGE - good for ATTACK';
            else if (enemyDist > 400) situation = 'LONG RANGE - close distance first';

            const prompt = `You are Blue Robot AI. Output ONE command only.

SITUATION: HP=${hp}% Energy=${energy} Enemy=${enemyAngle}° ${enemyDist}m
${situation ? 'Note: ' + situation : ''}
ORDERS: "${orders}"

COMMANDS:
- FIRE ${enemyAngle} ${enemyDist} (shoot at enemy)
- ATTACK (auto-aim fire)
- MOVE <angle> <speed> (drive, speed 50-100)
- EVADE (random dodge)
- GOTO ENEMY (chase)
- GOTO BASE (retreat to heal)

Command:`;

            // Use BBS route -> router_agent -> BitNet (persistent CLI)
            commandQueryStartTime = Date.now();
            try {
                await window.bbs.postEvent({
                    subject: 'RouterQuery',
                    body: JSON.stringify({
                ...
                debugLog('📤 Command sent to router_agent (BitNet)');
                startEvolutionPolling();
            } catch (error) {
                debugLog('❌ Command post failed: ' + error.message);
                resetEvolutionUI();
            }
        }
        ...
        // ═══════════════════════════════════════════════════════
        // Guardrails: Validate AI commands to only allow safe patterns
        // ═══════════════════════════════════════════════════════
        function validateRobotCommand(rawOutput) {
            if (!rawOutput) return null;
            const output = rawOutput.trim().toUpperCase();

            // Extract first VALID command letter (S/C/D/R/M) from anywhere in response
            // This handles "Sure" → S, "Charge" → C, "Got it" → (none), etc.
            const validLetterMatch = output.match(/[SCDRM]/);
            const firstLetter = validLetterMatch ? validLetterMatch[0] : output.replace(/[^A-Z]/g, '').charAt(0);

            // S = Shoot (with optional coordinates)
            const shootMatch = output.match(/\bS(?:HOOT)?\s*(-?\d+)\s+(\d+)/);
            if (shootMatch) {
                const cmd = `FIRE ${shootMatch[1]} ${shootMatch[2]}`;
                debugLog('Validated S → FIRE: ' + cmd);
                return cmd;
            }
            // S = Shoot at enemy (no movement)
            if (firstLetter === 'S' || /\bSHOOT\b/.test(output)) {
                debugLog('Validated S → SHOOT');
                return 'SHOOT';
            }

            // C = Chase (fire + drive toward enemy)
            if (firstLetter === 'C' || /\bCHASE\b/.test(output)) {
                debugLog('Validated C → CHASE');
                return 'CHASE';
            }

            // D = Dodge (evasive maneuver)
            if (firstLetter === 'D' || /\bDODGE\b/.test(output)) {
                debugLog('Validated D → DODGE');
                return 'DODGE';
            }

            // R = Retreat (go to base)
            if (firstLetter === 'R' || /\bRETREAT\b/.test(output)) {
                debugLog('Validated R → RETREAT');
                return 'RETREAT';
            }

            // M = Mine (drop mine)
            if (firstLetter === 'M' || /\bMINE\b/.test(output)) {
                debugLog('Validated M → MINE');
                return 'MINE';
            }

            // Legacy command support
            if (/\bATTACK\b/.test(output)) return 'SHOOT';
            if (/\bEVADE\b/.test(output)) return 'DODGE';
            if (/\bGOTO\s+BASE\b/.test(output)) return 'RETREAT';

            // Fallback - reject invalid command
            debugLog('Invalid command rejected: ' + output.substring(0, 50));
            return null;
        }

        function handleEvolutionResponse(response) {
            // Log latency
            const latency = commandQueryStartTime > 0 ? Date.now() - commandQueryStartTime : 0;
            if (latency > 0) {
                debugLog(`⏱️ BitNet response in ${latency}ms`);
            }
        ...
        function resetEvolutionUI(message = 'Ready', color = '#6b7280') {
            ...
            if (evolveBtn) {
                ...
                evolveBtn.textContent = '⚡ Send Command';
            }
...
`;
        ...
        function startGame() {
            ...
            if (commanderPanel) {
                ...
                // Wire up prompt editor buttons
                const togglePromptBtn = document.getElementById('toggle-prompt-editor');
                const promptEditorSection = document.getElementById('prompt-editor-section');
                const autopilotPromptTextarea = document.getElementById('autopilot-prompt');
                const savePromptBtn = document.getElementById('save-prompt-btn');
                const resetPromptBtn = document.getElementById('reset-prompt-btn');

                // Load current prompt into textarea
                if (autopilotPromptTextarea && !autopilotPromptTextarea._promptLoaded) {
                    autopilotPromptTextarea.value = autopilotPrompt;
                    autopilotPromptTextarea._promptLoaded = true;
                }

                // Toggle editor visibility
                if (togglePromptBtn && !togglePromptBtn._toggleBound) {
                    togglePromptBtn.addEventListener('click', () => {
                        const isVisible = promptEditorSection.style.display !== 'none';
                        promptEditorSection.style.display = isVisible ? 'none' : 'block';
                        togglePromptBtn.textContent = isVisible ? 'Show ▼' : 'Hide ▲';
                    });
                    togglePromptBtn._toggleBound = true;
                }

                // Save prompt to localStorage
                if (savePromptBtn && !savePromptBtn._saveBound) {
                    savePromptBtn.addEventListener('click', () => {
                        autopilotPrompt = autopilotPromptTextarea.value;
                        localStorage.setItem('autopilot_prompt', autopilotPrompt);
                        debugLog('💾 Autopilot prompt saved!');
                    ...
                    savePromptBtn._saveBound = true;
                }

                // Reset to default prompt
                if (resetPromptBtn && !resetPromptBtn._resetBound) {
                    resetPromptBtn.addEventListener('click', () => {
                        autopilotPrompt = DEFAULT_AUTOPILOT_PROMPT;
                        autopilotPromptTextarea.value = DEFAULT_AUTOPILOT_PROMPT;
                        localStorage.setItem('autopilot_prompt', DEFAULT_AUTOPILOT_PROMPT);
                        debugLog('🔄 Autopilot prompt reset to default');

                        // Visual feedback
                        resetPromptBtn.textContent = '✅ Reset!';
                        setTimeout(() => {
                            resetPromptBtn.textContent = '🔄 Reset Default';
                        }, 2000);
                    });
                    resetPromptBtn._resetBound = true;
                }
            }
            ...
            debugLog('Game started!' + (autoPilotEnabled ? ' [AUTO-PILOT ACTIVE]' : ''));
        }

        // ═══════════════════════════════════════════════════════
        // Auto-Pilot System: Continuous AI command loop
        // ═══════════════════════════════════════════════════════
        let autoPilotWarmedUp = false; // Track if LLM has been warmed up

        async function warmUpLLM() {
            // Send context to LLM to pre-train it on robot command format
            const warmupPrompt = `Robot battle game. Commands: SCOUT (S) = move cautiously, CHARGE (C) = attack aggressively, DEFEND (D) = defensive position, RETREAT (R) = escape danger, MINE (M) = place mine. Respond with just the command letter (S, C, D, R, or M). Example: "H80 E300 @45/200 G" means Health=80%, Energy=300, enemy at 45 degrees 200 units away, Good situation. Best command?`;

            debugLog('🧠 Warming up LLM with game context...');
            ...
            try {
                ...
                // Don't wait for response, just send it for context
                await new Promise(r => setTimeout(r, 500)); // Brief delay
                debugLog('✅ LLM warmed up with context');
                autoPilotWarmedUp = true;
            } catch (error) {
                debugLog(`⚠️ Warmup failed: ${error.message}`);
            }
        }
        ...
        function startAutoPilot() {
            ...
            autoPilotWarmedUp = false; // Reset on new game
            debugLog('🤖 AUTO-PILOT: Starting continuous AI control for Blue robot (chained mode)');

            // Chain commands - next one starts immediately after previous completes
            async function autoPilotLoop() {
                // Warm up LLM on first start
                if (!autoPilotWarmedUp) {
                    await warmUpLLM();
                }

                while (autoPilotActive && currentState === GameState.PLAYING) {
                    await runAutoPilotCommand();
                    await new Promise(r => setTimeout(r, 2000)); // Prevent tight loop
                }
        ...
        function stopAutoPilot() {
            ...
            debugLog('🤖 AUTO-PILOT: Stopped');
        }

        async function runAutoPilotCommand() {
            if (isEvolving) {
                debugLog('🤖 AUTO-PILOT: Waiting...');
                return;
            }

            const robot = robots[0]; // Blue robot
            if (!robot || robot.damage >= 100) {
                debugLog('🤖 AUTO-PILOT: Robot unavailable');
                return;
            }
            ...
            // Tactical assessment
            let sit = 'G'; // Good
            if (hp < 30) sit = 'L';           // Low HP
            else if (energy < 150) sit = 'E'; // Low energy
            else if (enemyDist < 120) sit = 'C'; // Close
            else if (enemyDist > 350) sit = 'F'; // Far

            // Use custom autopilot prompt (editable via UI) with placeholder substitution
            const prompt = autopilotPrompt
                .replace(/{hp}/g, hp)
                .replace(/{energy}/g, energy)
                .replace(/{enemyAngle}/g, enemyAngle)
                .replace(/{enemyDist}/g, enemyDist)
                .replace(/{sit}/g, sit);
            ...
            try {
                // Post to BBS - router_agent picks up in ~8ms avg
                await window.bbs.postEvent({
                    subject: 'RouterQuery',
                    body: JSON.stringify({
                        query_id: evolveQueryId,
                ...
                debugLog(`📤 Query posted (${sseConnected ? 'SSE' : 'polling'} response expected in ~200ms)`);

                // If SSE connected, response arrives via handleSSERouterResponse()
                // If SSE not connected, poll for response directly
                if (!sseConnected) {
                    pollForRouterResponse(evolveQueryId);
                }

            } catch (error) {
                debugLog(`❌ AUTO-PILOT error: ${error.message}`);
                isEvolving = false;
                evolveQueryId = null;
            }
        }

        async function pollForRouterResponse(queryId, maxAttempts = 20, pollInterval = 100) {
            // Poll for RouterResponse via REST API (fallback when SSE unavailable)
            const startTime = Date.now();

            for (let attempt = 0; attempt < maxAttempts; attempt++) {
                try {
                    // Check if SSE took over (response already received)
                    if (!isEvolving || evolveQueryId !== queryId) {
                        debugLog(`⏹️ Polling stopped (query completed via SSE)`);
                        return;
                    }

                    // Use the BBS API to read events (works without SSE)
                    const eventsData = await window.bbs.readEvents();
                    // API returns array directly, not {messages: [...]}
                    const messages = Array.isArray(eventsData) ? eventsData : (eventsData.messages || []);

                    // Look for our RouterResponse
                    const routerResponse = messages.find(m => {
                        if (m.subject !== 'RouterResponse') return false;
                        try {
                            const body = JSON.parse(m.body);
                            return body.query_id === queryId;
                        } catch {
                            return false;
                        }
                    });
                    ...
                    if (routerResponse) {
                        ...
                        debugLog(`📥 Polling found response in ${(elapsed * 1000).toFixed(0)}ms (attempt ${attempt + 1})`);

                        try {
                            const responseBody = JSON.parse(routerResponse.body);
                            handleSSERouterResponse(responseBody);
                        } catch (e) {
                            debugLog(`❌ Failed to parse polled response: ${e.message}`);
                            isEvolving = false;
                            evolveQueryId = null;
                        }
                        return;
            ...
            // Timeout
            debugLog(`⏱️ Polling timeout after ${maxAttempts * pollInterval}ms (no response found)`);
            isEvolving = false;
            evolveQueryId = null;
        }
        ...
        function onCanvasClick(event) {
            ...
            if (intersects.length > 0) {
                ...
                // Handle menu overlay checkbox clicks
                if (clicked.userData.action === 'start' && menuOverlay && menuOverlay.visible) {
                    // Check if click is on the checkbox area (using UV coordinates)
                    const uv = intersects[0].uv;
                    if (uv) {
                        const canvasX = uv.x * 512;
                        const canvasY = (1 - uv.y) * 300;

                        // Checkbox is at (176, 255) with size 20x20
                        if (canvasX >= 176 && canvasX <= 196 && canvasY >= 255 && canvasY <= 275) {
                            autoPilotEnabled = !autoPilotEnabled;
                            debugLog('Auto-Pilot ' + (autoPilotEnabled ? 'ENABLED' : 'DISABLED'));
                            updateMenuOverlay();
                            return;
                        }

                        // Start button is at (156, 90) with size 200x50
                        if (canvasX >= 156 && canvasX <= 356 && canvasY >= 90 && canvasY <= 140) {
                            startGame();
                            return;
                        }
                    }
        ...
        function updateMenuOverlay() {
            ...
            if (autoPilotEnabled) {
                ...
                ctx.textAlign = 'center';
                ctx.fillText('✓', checkboxX + 10, checkboxY + 16);
            }

            ctx.fillStyle = '#9ca3af';
            ctx.font = '14px monospace';
            ctx.textAlign = 'left';
            ctx.fillText('AI Auto-Pilot (Blue)', checkboxX + 30, checkboxY + 15);
...
// BitNet v2.9.39 - Commander Console uses router_agent/BitNet (Updated 17:48)
